...
class NetBox(BaseBackend):
        ...
        for prefix in prefixes.values():
            new_prefix = self.obj_write(
                ...
                {"q": str(prefix.prefix)},
                {
                    ...
                    "vrf": vrfs_old_to_new.get(prefix.vrf_id, None) if prefix.vrf_id else None,
                },
                self.prefix_get,
            )
    ...
    def ip_addresses_write(self, ip_addresses, vrfs_new, vrfs_old_to_new):
        '''
        Write a dictionary of IPAddress objects to the API backend,
        using previously written VRFs to preserve reference information.
        '''
        ...
        for ip_address in ip_addresses.values():
            ...
            # Check if description is "DHCP" or "reserved" and update the status accordingly
            if "DHCP" in description.upper():
                status = "dhcp"
            elif "Reserved" in description.upper():
                status = "reserved"
    ...
    def ip_address_get(data):
        ...
        # Check if 'hostname' exists in data, and if so, use it as 'hostname'
        hostname = data.get("dns_name")  # Use get() to handle the case where 'hostname' might not exist
